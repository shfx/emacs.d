...
(defun project-try-packagejson (dir)
  (if-let ((root (if (listp "package.json")
                     (seq-some (lambda (n)
                                 (locate-dominating-file dir n))
                               "package.json")
...
(setq project-packagejson-ignore '("node_modules/" ".pnpm-store/" ".dist/" ".tsBuild/"))

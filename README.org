...
(use-package project
  :ensure nil
  :config
  (defvar project-packagejson-ignore '("node_modules/" ".pnpm-store/" ".dist/" ".tsBuild/"))
  
  (defun parent-node-modules-p (path)
    "Return non-nil if PATH is  `node_modules'"
    (let* ((absolute-path (expand-file-name path))
           (splitted-path (split-string absolute-path "/" t)))
      (and (seq-find (lambda (segment) (equal segment "node_modules")) splitted-path) t)))

  (defun project-try-local (dir)
    (when-let* ((root (locate-dominating-file dir ".project")))
      (cons 'local root)))

  (defun project-try-packagejson (dir)
    (when-let* ((root (locate-dominating-file dir "package.json"))
                (inside-node-modules (not (parent-node-modules-p dir))))
      (cons 'packagejson root)))

  (cl-defmethod project-root ((project (head local)))
    (cdr project))
  
  (cl-defmethod project-root ((project (head packagejson)))
    (cdr project))

  (cl-defmethod project-ignores ((project (head packagejson)) dir)
    (let* ((root (project-root project)))
      (append
       (project--value-in-dir 'project-packagejson-ignore root)
       (mapcar
        (lambda (dir)
          (concat dir "/"))
        vc-directory-exclusion-list))))
  (setq project-find-functions '(project-try-packagejson
                                 project-try-local
                                 project-try-vc)))
#+end_src

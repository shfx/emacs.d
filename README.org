...
(defun alerter--option (args)
  ...
  (lambda (key)
    (concat "-" (alerter--symbol-value key) " " (format "\"%s\"" (plist-get args key)))))
...
(defun alerter (message &rest args)
  ...
  (if (executable-find "alerter")
      (let* ((options (seq-filter (lambda (element) (keywordp element)) args))
...
(defun check-if-config-valid ()
  (alerter "Checking config..." :group "check-config" :timeout "60")
  (async-start (lambda ()
                 (call-process-shell-command "./scripts/validate"))
               (lambda (results)
                 (if (< 0 results)
                     (alerter "Problem with emacs config!" :group "check-config" :sound "default")
                   (alerter "Config valid!" :sound "default" :group "check-config" :timeout "5"))
                 )))

...
#+begin_src emacs-lisp
(defun my/remove-slant-attribute ()
...
(use-package kanagawa-theme
  ...
  :demand t
  :hook
  ;; lazy loaded modules can define slat faces, this removes them
  (custom-define . my/remove-slant-attribute)
  :config
  (load-theme 'kanagawa :no-confim)
  (set-face-attribute 'default nil :font "JetBrains Mono" :weight 'normal :height 160)
  ;; (my/remove-slant-attribute)
  (when (eq system-type 'darwin)
...
(use-package claude-code
  :ensure (claude-code :fetcher github :repo "stevemolitor/claude-code.el" :rev :newest)
...
#+begin_src emacs-lisp
(use-package monet
  :ensure (monet :fetcher github :repo "stevemolitor/monet" :rev :newest))
#+end_src
...
(use-package agent-shell
  ...
  ((claude . "brew install claude-code")
   (claude-code-acp . "mise install npm:@zed-industries/claude-code-acp"))
  :custom
...
(use-package lsp-mode
  ...
  (lsp-eslint-multi-root nil)
  (lsp-eslint-server-command (list "node" "--max-old-space-size=8192" (concat (expand-file-name user-emacs-directory) "var/lsp/server/eslint/unzipped/extension/server/out/eslintServer.js") "--stdio"))
  ...
  ;; disable eslint server check to override server command
...
;;   :ensure t
;;   :custom
;;   (md-ts-hide-markup nil)
;;   :custom-face
;;   (md-ts-delimiter ((t (:inherit default))))
;;   :init
;;   (md-ts-mode-enable-global)
;;   :config
;;   (defun md-ts--fontify-thematic-break (node override start end &rest _) nil))

(use-package grip-mode
  :ensure t
  :config (setq grip-command 'mdopen) ;; auto, grip, go-grip or mdopen

...
# Disable Language Selector Bubble
...
(define-advice keyboard-quit
    (:around (quit) keyboard-quit-dwim)
  (cond
   ((region-active-p)
...
** auth-source-1password ([[https://github.com/dlobraico/auth-source-1password][link]])

Integrates 1Password (via the ~op~ CLI) as an auth-source backend.
Requires the [[https://developer.1password.com/docs/cli/][1Password CLI]] to be installed and signed in.

Lookups use ~auth-source-pick-first-password :host ENTRY :user FIELD~,
resolving to the secret reference ~op://VAULT/ENTRY/FIELD~.

#+begin_src emacs-lisp
(use-package auth-source-1password
  :ensure (:host github :repo "dlobraico/auth-source-1password")
  :after auth-source
  :custom
...
(use-package emacs
  ...
  (scroll-step 1)
  (search-default-mode t ;; regex isearch
...
(defun my/increase-font-size ()
  ...
  (let* ((old-height (face-attribute 'default :height))
         (new-height (+ old-height 10)))
    (message (format "Changed font size to %d" new-height))
...
For the end-user, it provides a simple way to periodically run a
subset of `kill-emacs-hook', ensuring that state variables are written
to disk long before something happens to your Emacs session.

#+begin_src emacs-lisp
(use-package eager-state
  :ensure (eager-state :fetcher github :repo "meedstrom/eager-state")
  :custom
...
(define-advice enable-theme (:after (theme) +run-after-enable-theme-hook)
  "Run `after-enable-theme-hook'."
  (run-hook-with-args 'after-enable-theme-hook theme))
#+end_src
...
(defun debounce (func &optional delay)
  (let ((timer)
    ...
    (lambda (&rest args)
      (prog1 nil
        (if (timerp timer)
            (timer-set-idle-time timer delay)
          (setq timer
                (run-with-idle-timer
                 delay nil
                 (lambda (buf)
...
(defmacro defbounce (name arglist delay &rest body)
  (declare
   (doc-string 4) (indent 2))
  (let* ((parse (byte-run--parse-body body t))
         (docstring (nth 0 parse))
         (declare-form (nth 1 parse))
         (body (nth 3 parse)))
    (progn
      `(let ((debounced (debounce (lambda ,arglist ,@body) ,delay)))
         (defun ,name ,arglist
           ,docstring
           ,declare-form
           (funcall debounced ,@(seq-difference arglist '(&rest &optional))))))))

(defbounce +remove-slant-attribute (&optional theme) 0.05
           "Iterate over all faces and resets slant attributs to `':normal`'.
   Call it on custom-define-hook so any defered packages will be reset
   with this function."
           (declare (ftype (function (&optional string) nil)))
           (mapc (lambda (face)
...
(use-package kanagawa-theme
  :ensure (kanagawa-theme :fetcher github :repo "arouene/emacs-kanagawa-theme" :files ("kanagawa-theme.el"))
  :demand t
  :hook
  ((after-enable-theme custom-define) . +remove-slant-attribute)
  :preface
  (load-theme 'kanagawa :no-confim)
  :custom-face
  (default ((t (:font "JetBrains Mono" :weight normal :height 160))))
  (org-block ((t (:extend t :background "#16161D" :foreground "#DCD7BA" :slant normal))))
  (org-ellipsis ((t (:foreground "gray40" :underline unspecified))))
  :custom
  (kanagawa-theme-underline-parens nil)
  (kanagawa-theme-keyword-italic nil)
  (kanagawa-theme-comment-italic nil)
  (face-font-rescale-alist
   '(("Apple Color Emoji" . 0.85)
     ("Noto Color Emoji" . 0.85)
     ("Segoe UI Emoji" . 0.85)
     ("Symbola" . 0.85)))
  :config
  (+remove-slant-attribute))
#+end_src
...
(use-package nerd-icons-completion
  :after corfu marginalia nerd-icons
...
(setopt pixel-resolution-fine-flag t)
...
(use-package tab-bar
  ...
  (tab-bar-separator (propertize "  " 'face 'tab-bar-separator))
  ...
  (tab-bar-tab-inactive ((t (:foreground "#54536D"))))
  (tab-bar-tab-highlight ((t (:inherit background))))
  (tab-bar-separator ((t (:foreground "#54536D" :height 1.2 :inherit tab-bar))))
  :config;
...
(use-package visible-auto-revert
  :ensure (:fetcher github :repo "kn66/visible-auto-revert.el")
  :custom
...
Highlights a line when switching buffers, windows and frames.

#+begin_src emacs-lisp
(use-package pulsar
  :custom
  ;; disable region operation highlights as we're using goggles-mode for that
...
(defmacro electric-pair-ignore-chars (mode &rest char-list)
  "Defines an electric pair ignore list for MODEs hook variable"
  (let ((name (intern (format "electric-pair-ignore-chars-%s" (symbol-name mode))))
        (hook (intern (format "%s-hook" (symbol-name mode)))))
    `(progn
      (defun ,name ()
         (add-function :before-until
                       (local 'electric-pair-inhibit-predicate)
                       (lambda (c) (memq c (list ,char-list)))))
      (add-hook ',hook #',name))))
#+end_src
...
(use-package meow
  ...
  (meow-motion-cursor ((t (:background "white" :inherit unspecified))))
...
(use-package paren
  :ensure nil
  :custom
  (show-paren-predicate '(and (not (derived-mode . special-mode))
                               (not (derived-mode . ghostel-mode))))
  :config
  (show-paren-mode 1))
#+end_src
...
(use-package vertico
  ...
  (defvar +vertico-default-action #'vertico-exit
    "`vertico-exit' or `vertico-directory-enter' are good choices.")

  (defun +embark-act-all-noconfirm ()
    "`embark-act-all' but never ask for confirmation."
    (interactive)
    (let (embark-confirm-act-all)
      (call-interactively #'embark-act-all)))

  (defun +vertico-actall-or-default ()
    "`embark-act-all' if there are selections otherwise run `+vertico-default-action'."
    (interactive)
    (if (bound-and-true-p embark--selection)
        (call-interactively #'+embark-act-all-noconfirm)
      (call-interactively +vertico-default-action)))

  (keymap-set vertico-map "C-SPC" #'embark-select)
  (keymap-set vertico-map "<return>" #'+vertico-actall-or-default))

(use-package vertico-posframe
  :after vertico
  :custom-face
  (vertico-posframe-border ((t (:background unspecified :inherit shadow))))
  :custom
...
(use-package ghostel
  ...
  (:map +ghostel-map
        ("v" . ghostel)
        ("p" . +ghostel-pi)
        ("r" . +ghostel-pi-restart))
  :custom
  (ghostel-module-auto-install 'download)
  (ghostel-detect-password-prompts nil)
  (ghostel-enable-osc52 t)
  :config
  (defvar +ghostel-map (make-sparse-keymap))
  (defvar-local +ghostel-pi-buffer nil)

  (defun +ghostel-pi ()
    (interactive)
    (let* ((name (if (projectile-project-p)
                     (concat "*pi - " (projectile-project-name) "*")
                   ghostel-buffer-name))
           (buffer (get-buffer-create name)))
      (with-current-buffer buffer
        (switch-to-buffer buffer)
        (setq-local ghostel-detect-password-prompts nil)
        (ghostel-exec buffer "/bin/zsh" '("-i" "-c" "pi"))
        (setq-local +ghostel-pi-buffer t))))

  (defun +ghostel-pi-restart ()
    (interactive)
    (while (and (eq major-mode 'ghostel-mode)
                (boundp '+ghostel-pi-buffer)
                (eq +ghostel-pi-buffer t))
...
;; run (eat-compile-terminfo) if shell smells funny
(use-package eat
  :hook
  (eshell-first-time-mode-hook . eat-eshell-mode)
  :custom
  (process-adaptive-read-buffering t)
  (eat-term-scrollback-size 300000)
  :config
  (setopt eat-semi-char-non-bound-keys (seq-difference eat-semi-char-non-bound-keys '([C-left] [C-right]))))
...
(use-package consult-jump-project
  :ensure (consult-jump-project :fetcher github :repo "jdtsmith/consult-jump-project")
  :custom
  (consult-jump-project-direct-jump-modes '(dired-mode))
...
This file documents TRAMP 2.8.1, a remote file editing package for Emacs.
...
local files. This extends to editing, version control, dired, and
more.

Added some changes based on this article:
...
#+begin_src emacs-lisp
  (use-package lsp-proxy
    :ensure (lsp-proxy :fetcher github :repo "jadestrong/lsp-proxy"))
#+end_src
  ...
  (treesit-language-source-alist
   (append
    '((hyprlang "https://github.com/tree-sitter-grammars/tree-sitter-hyprlang" nil nil nil nil)
      ...
      (svelte "https://github.com/themixednuts/tree-sitter-htmlx" "master" nil nil nil))
    treesit-language-source-alist))
  :config
  (setq treesit--install-language-grammar-out-dir-history (list (no-littering-expand-var-file-name "treesit"))))
#+end_src
...
#+begin_src emacs-lisp
(use-package lab
  :ensure (lab :fetcher github :repo "isamert/lab.el")
  :after magit
  :bind-keymap
  ("C-x l" . lab-map)
  :custom
  (lab-action-handler #'completing-read)
  (lab-host "https://gitlab.service.osa")
  (lab-group "274")
  :config
  (when (functionp 'magit-push-current-to-pushremote)
...
(use-package diff-mode
  ...
  :bind
  (:map diff-mode-map
        ("1" . (lambda () (interactive) (outline-hide-sublevels 1)))
        ("2" . (lambda () (interactive) (outline-hide-sublevels 2)))
        ("3" . #'outline-show-all)))
#+end_src
...
(use-package yasnippet
  ...
  ;; We don't want to expand non keywords, like a function call chain
  ;; This will go back all the alpha chars back and see if there is a
  ;; whitespace or an
  (yas-buffer-local-condition
   (lambda ()
     (save-excursion
       (skip-chars-backward "a-zA-Z")
       (when (or (bobp) (memq (char-before) '(?\n ?\s ?\t ?\()))
         t))))
  :config
  (yas-reload-all))
...
(use-package prisma-mode
  ...
  :hook (prisma-mode . lsp-proxy-mode))
#+end_src
...
(use-package rustic
  ...
  :hook
  (rustic-mode-hook . (rustic-flycheck-setup rustic-mode-auto-save-hook))
  :custom
...
(use-package typescript-mode
  ...
  ("\\.tsx$" . tsx-ts-mode)
  :hook ((typescript-ts-mode tsx-ts-mode) . lsp-proxy-mode))
...
(use-package svelte-ts-mode
  ...
  :hook (svelte-ts-mode . lsp-proxy-mode)
  :custom
  (treesit-language-source-alist
   (append
    '((svelte
      ...
      (typescript
       . ("https://github.com/tree-sitter/tree-sitter-typescript" nil "typescript/src"))
      (javascript
...
(use-package dockerfile-mode
  :hook (dockerfile-mode . lsp-proxy-mode)
  :mode "Dockerfile$")

(use-package docker-compose-mode
  :hook (docker-compose-mode . lsp-proxy-mode)
...
#+begin_src emacs-lisp
(use-package pgmacs
  :ensure (pgmacs :fetcher github :repo "emarsden/pgmacs")
  :defer t)
